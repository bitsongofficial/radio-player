...
export const parseErrorResponseKeplr = (response) => {
  let parsedResponse = {
    success: false,
    tx_hash: null,
    log: null
  ...
  if (response.logs === undefined) {
    parsedResponse.log = response.rawLog
  } else {
    parsedResponse.success = true
    parsedResponse.tx_hash = response.transactionHash
  }
...
export const defaultFees = () => {
  return {
    amount: coins(
      new BigNumber(process.env.DEFAULT_GAS_PRICE).multipliedBy(new BigNumber(process.env.DEFAULT_GAS_LIMIT)).toNumber(),
      process.env.MICROSTAKEDENOM.toLowerCase()
    ),
    gas: process.env.DEFAULT_GAS_LIMIT
  };
}

export const enableKeplr = async (address) => {
  try {
